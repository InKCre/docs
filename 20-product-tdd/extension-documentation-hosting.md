...
bytes. DNS routing and TLS coverage may intentionally accept a wider hostname set than the
documentation address template. Registry must still admit only the exact host pattern generated by
that template; resolving through the same wildcard or completing TLS does not make another host a
documentation origin. Concrete domain, DNS, TLS, and storage deployment choices remain
Registry-owned.
