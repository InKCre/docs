...
1. [Prepare the Extension](/guide/extensions): `inkcre/rss` version `0.2.1` for Core Host SDK
   `0.3.x`. Follow the Registry-to-Web installation steps, then select your online Core Peer under
   **Extensions → Run on Peer** and enable it there. No CLI or browser Extension is needed for this
   collector.
...
Web app, select Core under **Extensions → Run on Peer** to control the collector, not **This
browser**.
