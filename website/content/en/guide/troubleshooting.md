...
| Core slow or offline                     | In Settings → Clients, choose **Wake**, then refresh. Inspect host logs if it stays offline.              |
...
| CLI works but Web cannot find a provider | Wake Core from Settings, refresh Clients, and check that browser and Core Peer IDs differ.                |
